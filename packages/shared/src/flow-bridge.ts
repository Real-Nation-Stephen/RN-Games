...
export const FLOW_EXPERIENCE_STEP_CHANGED = "rngames:experience_step_changed";
...
export function emitStepComplete(outcomes: Record<string, unknown> = {}): void {
  if (typeof window === "undefined") return;
  const ctx = loadFlowContext() ?? parseFlowContextFromSearch(new URLSearchParams(window.location.search));
  const courseCtx = resolveCourseContext(new URLSearchParams(window.location.search));
  const payload = {
...
export type StepContentReadyMessage = {
  ...
  /** Landing is showing the flow completion / override view. */
  flowEndScreen?: boolean;
};
...
export type ExperienceStepChangedMessage = {
  type: typeof FLOW_EXPERIENCE_STEP_CHANGED;
  courseSessionId?: string;
  courseItemId?: string;
  isLastFlowStep?: boolean;
};

export function isExperienceStepChangedMessage(data: unknown): data is ExperienceStepChangedMessage {
  return (
    !!data &&
    typeof data === "object" &&
    (data as ExperienceStepChangedMessage).type === FLOW_EXPERIENCE_STEP_CHANGED
  );
}
