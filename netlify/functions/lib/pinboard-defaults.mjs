...
export const PINBOARD_DEFAULT_STICKIES = [
...
export function ensurePinboardGuestAssets(doc) {
  if (!doc.mobile) doc.mobile = {};
  const stickyWithUrl = (doc.mobile.stickyAssets || []).filter((s) => s.imageUrl?.trim());
  if (!stickyWithUrl.length) {
    doc.mobile.stickyAssets = PINBOARD_DEFAULT_STICKIES.map((s) => ({ ...s }));
    doc.stickies = doc.mobile.stickyAssets.map((s) => ({ ...s }));
  } else if (!Array.isArray(doc.stickies) || !doc.stickies.length) {
    doc.stickies = stickyWithUrl.map((s) => ({ ...s }));
  }
  const frames = doc.mobile.photoFrames || [];
  if (!frames.length) {
    doc.mobile.photoFrames = PINBOARD_DEFAULT_FRAMES.map((f) => ({ ...f }));
  } else if (!frames.some((f) => f.id === "none")) {
    doc.mobile.photoFrames = [{ id: "none", label: "No frame", imageUrl: "" }, ...frames];
  }
  const stickers = (doc.mobile.photoStickers || []).filter((s) => s.imageUrl?.trim());
  if (!stickers.length) {
    doc.mobile.photoStickers = PINBOARD_DEFAULT_PHOTO_STICKERS.map((s) => ({ ...s }));
  }
  return doc;
}
